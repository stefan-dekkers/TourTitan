...
export class RideService {
  ...
  validateRide(ride: CreateRideDto): boolean {
    ...
    this.logger.debug(`Validating ride: Departure DateTime - ${departureDateTime.toISOString()}, Current DateTime - ${currentDateTime.toISOString()}, Tomorrow - ${tomorrow.toISOString()}`);
  
    const isValid = (departureDateTime > currentDateTime && departureDateTime.toDateString() === currentDateTime.toDateString()) || (departureDateTime.toDateString() === tomorrow.toDateString());
  
    if (!isValid) {
      this.logger.warn(`Ride validation failed: Departure datetime ${departureDateTime.toISOString()} is not allowed. It must be later than the current time if today or any time if tomorrow.`);
    }
  ...
  async createRideLocation(ride: CreateRideDto): Promise<ILocation> {
    if (!ride.arrivalLocation || !ride.arrivalLocation.zipCode || !ride.arrivalLocation.number) {
      throw new ConflictException('Location information is required to create a car');
    }
  
    let location = await this.locationRepository.findOne({
      where: { zipCode: ride.arrivalLocation.zipCode, number: ride.arrivalLocation.number },
    });
  
    if (!location) {
      this.logger.debug('Creating a new location');
      location = await this.locationRepository.save(this.locationRepository.create(ride.arrivalLocation));
    }
  ...
  async create(ride: CreateRideDto): Promise<IRide | null> {
    ...
    throw new ConflictException(`Invalid departureTime: ${departureDateTime.toISOString()} is not within the allowed range (Today: ${currentDateTime.toISOString()}, Tomorrow: ${tomorrow.toISOString()})`);
    }
    
    // Check if the vehicle exists and is available
    const vehicle = await this.carRepository.findOne({where : { id: ride.vehicle.id }});
    ...
    const newRide = await this.rideRepository.save(this.rideRepository.create(ride));
  
    await this.carRepository.update({ id: newRide.vehicle.id }, { isAvailable: false });
  ...
  async finishRide(rideId: string, driverId: string, newMileage: number, arrivalTime :Date): Promise<IRide> {
    ...
    console.log(`arrivalTime: ${new Date (arrivalTime)} --> Ride arrivalTime ${ride.arrivalTime}`);
    if(new Date (arrivalTime) < ride.departureTime){
      throw new ConflictException('arrivalTime needs to be later than departureTime');
    }
    let mileage= 0;
    const vehicle = await this.carRepository.findOne({where : {id: ride.vehicle.id}});
    if (vehicle) {
      ...
      {
        throw new ConflictException('newMileage needs to be greater than vehicle mileage'); 
      }
