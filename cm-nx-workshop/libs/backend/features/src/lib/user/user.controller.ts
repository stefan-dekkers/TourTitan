...
export class UserController {
   ...
   async getAll(): Promise<IUser[]> {
    ...
    @ApiOkResponse({ description: 'Successfully retrieved user', type: CreateUserDto })
   ...
   async getOne(@Param('id') id: string, @Req() req:any): Promise<IUser | null>  {
    ...
    async update(@Param('id') id: string, @Body() data: UpdateUserDto, @Req() req: any): Promise<IUser|null> {
      ...
      console.log(`Updating user: Param ID: ${id}, Requester ID: ${requester.sub}, Is Admin: ${isAdmin}`);
    ...
    deleteUser(@Param('id') id: string, @Req() req:any): any {
      ...
      const isAdmin = requester.role === 'admin';
      console.log('Requester object:', requester);
      console.log(`Deleting user: Param ID: ${id}, Requester ID: ${requester.sub}, Is Admin: ${isAdmin}`);
      // Controleer of de gebruiker admin is of de eigenaar van het account
      if (!isAdmin && requester.sub !== id) {
          throw new ForbiddenException('Je bent niet gemachtigd om deze actie uit te voeren.');
      }
